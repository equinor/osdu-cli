...
from osducli.prompt import (verify_is_a_tty, NoTTYException, _INVALID_PASSWORD_MSG, prompt,
    ...
    def test_prompt_msg_empty_response_default_replacement_maxlen(self, _):
        ...
        with mock.patch('osducli.prompt._input', return_value=user_input) as _input_mock:
            with mock.patch('sys.stdout', new_callable=StringIO):
                ...
                self.assertEqual(expected_result, actual_result)
                _input_mock.assert_called_once_with(f'Please enter some text [{expected_displayed_default}]: ')
